...
class CatalogBloc {
  ...
  void _handleIndexes(List<int> indexes) {
    ...
    _pages.removeWhere((pageIndex, _) =>
        pageIndex < minPageIndex - CatalogService.productPerPage ||
        pageIndex > maxPageIndex + CatalogService.productPerPage);
  }

  /// -----------------------------------------------
  int _getPageStartFromIndex(int index) =>
      (index ~/ CatalogService.productPerPage) * CatalogService.productPerPage;

  /// -----------------------------------------------
  /// Handles arrival of a new [page] from the network. Will call [_sendNewSlice].
  _handleNewPage(CatalogPage page, int index) {
    _pages[index] = page;
...
class CatalogProvider extends InheritedWidget {
  ...
  CatalogProvider({Key key, @required CatalogBloc catalog, Widget child})
      : assert(catalog != null),
        catalogBloc = catalog,
  ...
  static CatalogBloc of(BuildContext context) =>
      context.dependOnInheritedWidgetOfExactType<CatalogProvider>().catalogBloc;
}
